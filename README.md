...
Run `python3 web_server.py`, then open `http://127.0.0.1:8765` in a browser. Select Hacker News and/or Hugging Face Daily Papers, refresh the live feed, choose up to five items, choose Luna (lower cost) or Terra (deeper synthesis), then write a source-linked episode. Click **Create MP3 with Mac voice** to synthesize audio with macOS `say` and `ffmpeg`; play it on the page or download the MP3 for your phone. This audio step does not make another model request. The OpenAI key stays in the local, ignored `.env` file; each click on **Write my episode** makes one paid API request. This local server is not intended for public deployment.

The AI writing prompt lives in `prompts/podcast_v1.txt`. It asks for one continuous, conversational podcast script with a unifying question and natural transitions. Article titles and source links stay in the notes, not in the spoken audio. The input still consists of headlines and abstracts rather than full papers, so the episode should be treated as an informed introduction, not a verified paper review.
