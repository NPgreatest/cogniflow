...
Run `python3 web_server.py`, then open `http://127.0.0.1:8765` in a browser. Select Hacker News and/or Hugging Face Daily Papers, refresh the live feed, choose up to five items, choose Luna (lower cost) or Terra (deeper synthesis), then generate a source-linked report. Click **Create MP3 with Mac voice** to synthesize audio with macOS `say` and `ffmpeg`; play it on the page or download the MP3 for your phone. This audio step does not make another model request. The OpenAI key stays in the local, ignored `.env` file; each click on **Generate report** makes one paid API request. This local server is not intended for public deployment.
